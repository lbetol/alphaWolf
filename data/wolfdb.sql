-- Host: localhost    Database: wolfdb
-- ------------------------------------------------------
--
...
create database if not exists wolfdb;
use wolfdb;

set global sql_mode=(select replace(@@sql_mode,'ONLY_FULL_GROUP_BY',''));
-- create user if not exists abox@localhost identified by 'Nutshe11';
-- grant all privileges on wolfdb.* to abox@localhost;
flush privileges;
SET FOREIGN_KEY_CHECKS=1;

alter schema wolfdb default character set utf8 default collate utf8_general_ci;
...
create table if not exists UFs( -- estados
	code	char	(32) unique not null,
    name	Varchar	(64)				, -- nome do estado
	shrt	char	(2)					, -- Sigla do estado
	primary key (code)
)engine=innodb;
...
create table if not exists Cities( -- cidades
	code	char	(32) unique not null,
	name	varchar	(64)				, -- nome da cidade
    uf		char	(32)				, -- código do estado da cidade
    primary key (code)					,
    foreign key	(uf)	references	UFs	(code)
)engine=innodb;
...
create table if not exists Engineers(  -- ANALISTA
  code				char	(32) unique not null,
  stts 				int		(1)	default 1		,
  name				varchar	(64),  -- nome do analista
  adrs				varchar	(64),  -- endereço
  email 			varchar	(64),  -- email
  date				char	(10),  -- data de que foi realizado o cadastro
  genre           	char 	(64),
  maritical_status	char 	(64),
  date_birth      	char 	(64), 
  complement      	varchar (64),
  num             	char 	(64),		
  district        	varchar (64),
  city				char	(32),  -- codigo da cidade
  uf				char	(32),  -- codigo do estado (UF)
  cep       		char	(8)	,
  tel				char	(17),  -- telefone
  cel				char	(18),  -- celular
  staff 			char	(32),  -- código do usuário
  primary key (code),
  foreign key (city) 	references Cities 	(code),
  foreign key (uf) 		references UFs 		(code),
  foreign key (staff) 	references Users	(code)
)engine=innodb;

-- drop table if exists Managers;
create table if not exists Managers(  -- GERENTE
  code				char	(32) unique not null,
  stts 				int		(1)	default 1		,
  name				varchar	(64),  -- nome do analista
  adrs				varchar	(64),  -- endereço
...
-- drop table if exists DbaS;
create table if not exists DbaS(  -- Analista de banco de dados
  code				char	(32) unique not null,
  stts 				int		(1)	default 1		,
  name				varchar	(64),  -- nome do analista
  adrs				varchar	(64),  -- endereço
...
create table if not exists Designs(  -- FRONT-END, DESIGN DO PROJETO
  code				char	(32) unique not null,
  stts 				int		(1)	default 1		,
  name				varchar	(64),  -- nome do analista
  adrs				varchar	(64),  -- endereço
...
create table if not exists Developers(  -- DESENVOLVEDOR, PROGRAMADOR 
  code				char	(32) unique not null,
  stts 				int		(1)	default 1		,
  name				varchar	(64),  -- nome do analista
  adrs				varchar	(64),  -- endereço
...
code  	char   (32)unique not null,
stts 	int    (1) default 1, -- status, se este cadastro esta ativo ou não
date 	char   (10), -- data que foi realizado o cadastro
user  	char   (32), -- usuário que realizou o cadastro
wolf	char   (32), -- usuário que receberá o acesso
modu	varchar(32), -- modulo que recebera o acesso
lvel	int		(1), -- nivel de acesso - 0 sem acesso; 1 visualizar; 2 cadastrar; 3 visualizar e cadastrar
primary key (code),
foreign key (user) references Users (code),
foreign key (wolf) references Users (code)
)engine=innodb;

-- drop table if exists TeamPacks;
create table if not exists TeamPacks( -- equipe - alcateia
code  	char   (32)unique not null,
stts 	int    (1) default 1, -- status, se este cadastro esta ativo ou não
date 	char   (10), -- data que foi realizado o cadastro
user  	char   (32), -- usuário que realizou o cadastro
name  	varchar(64), -- nome da equipe
primary key (code),
foreign key (user) references Users (code)
)engine=innodb;

insert into TeamPacks set 
code='teamaboxaboxaboxaboxaboxaboxabox', 
stts='0', 
date='00/00/0000',
user='aboxaboxaboxaboxaboxaboxaboxabox', 
name='Sem Equipe';

-- drop table if exists Teams;
create table if not exists Teams( -- membros da equipe
code  	char   (32)unique not null,
stts 	int    (1) default 1, -- status, se este cadastro esta ativo ou não
date 	char   (10),
tepk  	varchar(32), -- nome da equipe
mana 	char   (32)default null, -- membros da equipe
desi 	char   (32)default null, -- membros da equipe
deve 	char   (32)default null, -- membros da equipe
engi 	char   (32)default null, -- membros da equipe
dbas 	char   (32)default null, -- membros da equipe
user  	char   (32), -- usuário que realizou o cadastro
primary key (code),
foreign key (mana) references Managers	 (code),
foreign key (desi) references Designs	 (code),
foreign key (deve) references Developers (code),
foreign key (engi) references Engineers  (code),
foreign key (dbas) references DbaS 	  	 (code),
foreign key (user) references Users 	 (code),
foreign key (tepk) references TeamPacks  (code)
)engine=innodb;


create or replace view lobinhos as select
 Teams.tepk 	 as IDAlcateia,
 TeamPacks.name  as Alcateia,
 Engineers.code as code,
 Teams.stts 	 as stts,
 Managers.name   as gerente,
 Designs.name    as design,
 Developers.name as desenvolvedor,
 Engineers.name  as analista,
 DbaS.name       as dba
 from Teams
	inner join Managers   on Managers.code   = Teams.mana
    inner join Designs    on Designs.code 	 = Teams.desi
    inner join Developers on Developers.code = Teams.deve
    inner join Engineers  on Engineers.code  = Teams.engi
    inner join DbaS 	  on DbaS.code 		 = Teams.dbas
    inner join TeamPacks  on TeamPacks.code  = Teams.tepk
;

create or replace view wolfMan as select
 Teams.tepk 	as IDAlcateia,
...
create or replace view wolfDesi as select
 Teams.tepk 	as IDAlcateia,
 Designs.code 	as code,
 TeamPacks.name as Alcateia,
 Teams.stts 	as stts,
...
create or replace view wolfDeve as select
 Teams.tepk 	 as IDAlcateia,
 Developers.code as code,
 TeamPacks.name  as Alcateia,
 Teams.stts 	 as stts,
 Developers.name as name
 from Teams
    inner join Developers on Developers.code = Teams.deve
    inner join TeamPacks  on TeamPacks.code  = Teams.tepk
;

create or replace view wolfEng as select
 Teams.tepk 	as IDAlcateia,
 Engineers.code as code,
 TeamPacks.name as Alcateia,
 Teams.stts 	as stts,
...
create or replace view wolfDba as select
 Teams.tepk 	as IDAlcateia,
 DbaS.code 	    as code,
 TeamPacks.name as Alcateia,
 Teams.stts 	as stts,
...
create table if not exists Architectures( -- arquiteura do projeto
code char   (32) not null unique,
stts int    (1) default 1, -- status, se este cadastro esta ativo ou não
user char   (32) , -- código do usuário que está cadastrando a fase do projeto
date char	(10) ,
name varchar(64) , -- nome da arquitetura
dscr varchar(256), -- descrição da arquitetura 
primary key (code),
foreign key (user)  references Users  (code)
)engine=innodb;

-- drop table if exists Stages;
create table if not exists Stages( -- Fases da arquitetura, fase que o projeto terá
code char   (32) unique not null,
stts int    (1) default 1, -- status, se este cadastro esta ativo ou não
date char   (10), -- data que a fase foi cadastrado
user char   (32), -- código do usuário que esta realizando a operação 
arch char   (32), -- código da arquitetura
name varchar(64), -- nome da fase
numf int    (2) , -- número da fase 
decr varchar(128), -- descrição da fase
primary key (code),
foreign key (user) references Users         (code),
foreign key (arch) references Architectures (code)
)engine=innodb; 

-- alter table Stages 		  add foreign key (arch)  references Architectures (code);
-- alter table Architectures add foreign key (stage) references Stages 	   (code);


-- drop table if exists Projects;
create table if not exists Projects(  -- PROJETO
  code	 char	(32) unique not null,
  stts 	 int    (1) default 1, -- status, se este cadastro esta ativo ou não
  date 	 char	(10) , -- data de criação do projeto
  name	 varchar(64) , -- nome do projeto
  arch	 char 	(32) , -- código da arquitetura
  user 	 char	(32) , -- código do usuário 
  dscr	 varchar(512), -- descrição do projeto, introdução
  prbl	 varchar(512), -- pq o projeto será feito, qual o problema que ele resolverá
  jstf	 varchar(512), -- justificativa do projeto 
  objt	 varchar(512), -- objetivo a ser alcançado com o projeto	
  tpck	 char 	(32)  default null, -- código da equipe (matilha) que irá atuar no projeto 
  primary key (code),
  foreign key (tpck)  references TeamPacks 	   (code),
  foreign key (user)  references Users 		   (code),
  foreign key (arch)  references Architectures (code)
)engine=innodb;

-- drop table if exists ProjectStages;
create table if not exists ProjectStages( -- Fase do projeto - Controle de fase do projeto - 
  code	char	(32) unique not null,  -- código de identificação unico
  stts 	int    	(1) default 1,	-- status, se este cadastro esta ativo ou não
  date 	char	(10),   -- data de criação do projeto
  curr	char	(32),	-- fase atual do projeto ---- código da chave estrangeira da tabela Stages
  user  char 	(32), 	-- código do usuário que esta realizando a operação 
  vers	varchar (64),	-- versão que o projeto se encontra
  proj	char 	(32),	-- código do projeto
  primary key(code),
  foreign key (user)  references Users   (code),
  foreign key (curr)  references Stages  (code),
  foreign key (proj)  references Projects(code)
)engine=innodb;


create or replace view faseatual as select
	Stages.code as codestg,
	Stages.arch as codearcstg,
	Stages.name as nome,
	Architectures.name as arquitetura,
	ProjectStages.code as code,
	ProjectStages.curr as atual,
	ProjectStages.proj as projeto
from ProjectStages
	inner join Stages        on Stages.code   		= ProjectStages.curr
    inner join Architectures on Architectures.code  = Stages.arch
    inner join Projects      on Projects.code  		= ProjectStages.proj
;


-- drop table if exists ProjectCurrentStages;
create table if not exists ProjectCurrentStages(
  code	char (32) unique not null,  -- código de identificação unico
  stts 	int  (1) default 1,	-- status, se este cadastro esta ativo ou não
  date 	char (10),   -- data de criação do projeto
  user  char (32), 	-- código do usuário que esta realizando a operação 
  pjst  char (32),	-- código da fase atual do projeto -- chave estrangeira da tabela ProjectStages
  next 	int,		-- quantidade de vezes que avançou	
  back 	int,		-- quantidade de vezes que retrocedeu
  proj	char (32),	-- código do projeto
  primary key(code),
  foreign key (user)  references Users 		   (code),
  foreign key (pjst)  references ProjectStages (code),
  foreign key (proj)  references Projects	   (code)
)engine=innodb;
...
create table if not exists Actors(
  ...
  date 	char	(10), -- data que foi realizado o cadastro
  user	char	(32), -- usuário que realizou o cadastro
  name  varchar (64), -- nome do ator ou usuario que irá utilizar o projeto
  task	varchar(128), -- tarefas que o ator irá realizar
  objv	varchar(128), -- objetivos que o ator realizará 
  cprj	char	(32), -- código do projeto
  primary key (code),
  foreign key (user) references Users   (code),
  foreign key (cprj) references Projects(code)
)engine=innodb;

-- drop table if exists ListActors;
create table if not exists ListActors(
  code	char(32)unique not null,
  date 	char(10), -- data que foi realizado o cadastro
  stts  int (1) default 1, -- status, se este cadastro esta ativo ou não
  user	char(32), -- usuário que realizou o cadastro
  acto	char(32), -- ator que será listado
  cprj	char(32), -- código do projeto
  primary key (code),
  foreign key (acto)references  Actors  (code),
  foreign key (cprj) references Projects(code)
)engine=innodb;

create or replace view atores as select
	Actors.code 		as codeacto,
	Actors.name 		as nome,
	ListActors.code 	as code,
    ListActors.cprj as cprj
from ListActors
	inner join Actors on Actors.code = ListActors.acto
;



drop table if exists FunctionalRequirements;
create table if not exists FunctionalRequirements( -- REQUISITOS FUNCIONAIS
  code	char 	(32) unique not null,
  stts 	int     (1) default 1, -- status, se este cadastro esta ativo ou não
  date	char 	(10), -- data que foi realizado o cadastro
  user	char 	(32), -- código do usuário que realizou o cadastro
  idfr	char 	(6) , -- id do requisito funcional
  name 	varchar (64),
  dscr	varchar	(64), -- descrição do requisito funcional
  prio	varchar	(5) , -- prioridade do requisito funcional, 1-ALTO, 2-MÉDIO, 3-BAIXO
  cprj	char	(32), -- código do projeto
  primary key (code),
  foreign key (user) references Users 	(code),
  foreign key (cprj) references Projects(code)
...
drop table if exists NoFunctionalRequirements;
create table if not exists NoFunctionalRequirements( -- REQUISITOS NÃO FUNCIONAIS
  code	char 	(32) unique not null,
  stts 	int     (1) default 1, -- status, se este cadastro esta ativo ou não
  date	char 	(10), -- data que foi realizado o cadastro
  user	char 	(32), -- código do usuário que realizou o cadastro
  name 	varchar (64),
  idfr	char 	(6) , -- id do requisito não funcional
  dscr	varchar	(64), -- descrição do requisito não funcional
  prio	varchar	(5) , -- prioridade do requisito funcional, 1-ALTO, 2-MÉDIO, 3-BAIXO
  cprj	char	(32), -- código do projeto
  primary key (code),
  foreign key (user) references Users 	(code),
...
drop table if exists RelationReqFuns;
create table if not exists RelationReqFuns( -- relação de requisitos
  code	char 	(32) unique not null,
  stts 	int     (1)  default 1, -- status, se este cadastro esta ativo ou não
  date	char 	(10), -- data que foi realizado o cadastro
  user	char 	(32), -- código do usuário que realizou o cadastro
  name 	varchar (64),
  corf	char	(32), -- código do requisito funcional
  cprj	char	(32), -- código do projeto
  primary key (code),
  foreign key (corf) references FunctionalRequirements (code),
  foreign key (cprj) references Projects(code)
)engine=innodb;

drop table if exists RelationReqNoFuns;
create table if not exists RelationReqNoFuns( -- relação de requisitos
  code	char 	(32) unique not null,
  stts 	int     (1)  default 1, -- status, se este cadastro esta ativo ou não
  date	char 	(10), -- data que foi realizado o cadastro
  user	char 	(32), -- código do usuário que realizou o cadastro
  name 	varchar (64),
  corf	char	(32), -- código do requisito não funcional
  cprj	char	(32), -- código do projeto
  primary key (code),
  foreign key (corf) references NoFunctionalRequirements (code),
  foreign key (cprj) references Projects(code)
)engine=innodb;

-- drop table if exists UseCase;
create table if not exists UseCase( -- CASO DE USO
  code	char 	(32)unique not null,
  stts 	int     (1) default 1, -- status, se este cadastro esta ativo ou não
  date	char 	(10), -- data que foi realizado o cadastro
  user	char 	(32), -- usuário que realizou o cadastro
  iduc	char 	(6) , -- id do caso de uso
  name	varchar	(64), -- nome do caso de uso
  dscr	varchar(128), -- descrição do caso de uso
  prio	varchar	(5) , -- prioridade do requisito funcional, 1-ALTO, 2-MÉDIO, 3-BAIXO
  latr	char	(32), -- lista de atores que fará parte do caso de uso
  cprj	char	(32), -- código do projeto
  primary key (code),
  foreign key (user) references Users		(code),
  foreign key (latr) references ListActors	(code),
  foreign key (cprj) references Projects	(code)
...
create table if not exists Schedules(  -- CRONOGRAMA
  code  char 	(32) unique not null,
  stts 	int  	(1) default 1, -- status, se este cadastro esta ativo ou não
  date  char 	(10), -- data de cadastro do cronograma
  user 	char 	(32),-- código do usuário que está cadastrando o cronograma
  proj	char 	(32),-- código do projeto 
  stge	char 	(32), -- código da fase
  dtst  char 	(10), -- data de inicio da fase
  dtfs  char 	(10), -- data de término da fase
  stta  varchar (16), -- status, se esta fase está atrasado, em dia, pendente...
  primary key (code),
  foreign key (user) references Users 	 (code),
  foreign key (proj) references Projects (code),
  foreign key (stge) references Stages 	 (code)
)engine=innodb;


create or replace view matilhas as
select
	Projects.code codeproj,
	Projects.tpck as codepteam,
    TeamPacks.code as codeteam,
    TeamPacks.name as name
	from TeamPacks
    inner join Projects where TeamPacks.code = Projects.tpck;



create or replace view relass as select
  RelationReqFuns.code,
  RelationReqFuns.corf	as codefunc,
  RelationReqFuns.cprj  as codeproj,
  FunctionalRequirements.idfr as num
  from RelationReqFuns
	inner join FunctionalRequirements where FunctionalRequirements.code = RelationReqFuns.corf;
    
insert into UFs(code,name,shrt) values
(1,  "Acre", 				"AC"),
(2,  "Alagoas", 			"AL"),
(3,  "Amazonas", 			"AM"),
(4,  "Amapá", 				"AP"),
...
insert into Cities (code,name,uf) values
(1, "Afonso Cláudio", 8),
