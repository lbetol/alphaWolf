...
<div id="engmd" class="ab-window bwindow">
    <div class="ab-restore ttlecolor fntcolor"><icon>&#x24;</icon> <?=$new?"NOVO":"EDITAR"?> ANALISTA <icon>&#x24;</icon> Data de cadastro: <div class="ab-date"></div></div>
    <nav class="ab-controlbox">
            ...
            <icon class="zpd fn fblack cur hrs rt ab-tooltip" data-content="Salvar" onclick="ab_apply(function(){
                ab_loading(true);
                var stts = true;
                $('#engmd [required]').each(function(){$(this).checkout(function(d){if(!d) stts=false; })});
                   setTimeout(function(){
                    console.log(stts);
                    if(!stts){
    ...
    <div class="stretch ab-rise-engmd ab-scrollscope">
        ...
        <section class="abs ab-w75% hf ns" style="top:5vh;left:25%;border:1px dotted lightgray">
            <div class="ab-innerscrollable ab-scope">
                ...
                <fieldset class="sbar ab-anchor tct xbs">
                        ...
                        <div class="xts tct">
                            <div><input name="name" type="text" class="required" data-control="engmd;name"><label>Nome completo</label></div>
                        </div>
                </fieldset>
                ...
                <fieldset class="sbar ab-anchor tct xbs">
                    ...
                    <fieldset class="sbar ab-anchor tct xbs">
                            ...
                            <div class="ab-w80% xts tct">
                                ...
                                <div><input type="tel"   name="tel"   data-control="engmd;tel">   <label>Telefone</label></div>
                                <div><input type="tel"   name="cel"   data-control="engmd;cel">   <label>Celular</label> </div><br>
                                <div><input type="email" name="email" data-control="engmd;email"> <label>Email</label>   </div>
                            </div>
                    </fieldset>
                    <div class="wf hf">.</div>
                </section>   
    ...
    <script>
        ...
        <?php
            if($new){?>
                engmd.attr('code',ab_newId(32));
                engmd.attr('date',(new ab_Date()).today());
                engmd.attr('staff',abox.USER);
        <?php
            }else{?>
                ab_updateControllers("engmd");
                $("#input").attr("readonly",true);
                <?php
            }
            if((int)post("view")){?>
                $("select, input, textarea","#engmd").attr("readonly",true);
                <?php
            } ?> 
